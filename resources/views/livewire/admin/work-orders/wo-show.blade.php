...
<x-ui.page eyebrow="Órdenes"
    :title="'WO '.($po?->wo ?? $workOrder->wo_number)"
    :subtitle="trim(($part?->number ?? '').' — '.Str::limit($part?->description ?? '', 60), ' —') ?: 'Detalle de la orden de trabajo.'"
    back="{{ route('admin.work-orders.index') }}" backLabel="Volver a órdenes de trabajo">
        ...
        <div class="-mb-px flex flex-wrap gap-x-6 overflow-x-auto" role="tablist" aria-label="Secciones de la orden">
            @foreach ($tabs as $tab => $info)
                ...
                <button type="button" wire:key="wo-tab-{{ $tab }}" wire:click="setTab('{{ $tab }}')"
                    id="wo-tab-{{ $tab }}" role="tab"
                    aria-selected="{{ $isActiveTab ? 'true' : 'false' }}"
                    aria-controls="wo-panel-{{ $tab }}"
            ...
            <div class="space-y-5 lg:col-span-2">
                <x-ui.section title="Información de la orden"
                    ...
                    <dl class="divide-y divide-slate-200 rounded-lg border border-slate-200 dark:divide-slate-700 dark:border-slate-700">
                        ...
                        <x-ui.kv label="Cantidad pendiente" :value="number_format($workOrder->pending_quantity)"
                            :tone="$workOrder->pending_quantity > 0 ? 'warn' : 'good'" />
                ...
                @if ($po)
                    <x-ui.section title="Documento firmado"
                        ...
                        @if ($po->signed_document_path)
                            <div class="mb-4 flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between dark:border-slate-700 dark:bg-slate-900/40">
                                <div class="flex min-w-0 items-center gap-3">
                                    <svg class="size-8 shrink-0 text-red-500" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                        <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clip-rule="evenodd"/>
                                    </svg>
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-semibold text-slate-900 dark:text-white">{{ basename($po->signed_document_path) }}</p>
                                        <p class="text-xs text-slate-500 dark:text-slate-400">Documento firmado cargado</p>
                                    </div>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    <x-ui.btn variant="secondary" size="sm" href="{{ Storage::url($po->signed_document_path) }}"
                                        :navigate="false" target="_blank">Ver</x-ui.btn>
                                    <x-ui.btn variant="secondary" size="sm" href="{{ Storage::url($po->signed_document_path) }}"
                                        :navigate="false" download>Descargar</x-ui.btn>
                        ...
                        <x-ui.field for="signedDocument"
                            :label="$po->signed_document_path ? 'Reemplazar documento firmado' : 'Subir documento firmado'"
                            hint="Máximo 10 MB. Formatos: PDF, JPG o PNG."
                            :error="$errors->first('signedDocument')">
            ...
            <div class="space-y-5">
                <x-ui.section title="Cambiar estado"
                    ...
                    @can(\App\Livewire\Admin\StatusesWO\StatusWOManager::PERMISSION_VIEW)
                        <x-slot:aside>
                            <x-ui.btn variant="secondary" size="sm" wire:click="$dispatch('open-statuses-wo-manager')"
                        ...
                        @foreach ($statuses as $status)
                            @if ($status->id === $workOrder->status_id)
                                <div wire:key="wo-status-{{ $status->id }}"
                                    ...
                                    <span class="size-3 shrink-0 rounded-full" style="background-color: {{ $status->color }}" aria-hidden="true"></span>
                                    <span class="text-sm font-bold text-slate-900 dark:text-white">{{ $status->name }}</span>
                                    <span class="ml-auto text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">Actual</span>
                                </div>
                            @else
                                <button type="button" wire:key="wo-status-{{ $status->id }}"
                                    ...
                                    <span class="size-3 shrink-0 rounded-full" style="background-color: {{ $status->color }}" aria-hidden="true"></span>
                                    <span class="text-sm text-slate-700 dark:text-slate-200">{{ $status->name }}</span>
                                </button>
                            @endif
                        @endforeach
                    </div>
                    ...
                    <x-ui.note tone="muted" class="mt-4">
                        ...
                        Reábrelo con <strong>Open</strong> para que vuelva a aparecer en Capacidad y en la Lista de Envío.
                    </x-ui.note>
                </x-ui.section>
                ...
                <x-ui.section title="Progreso" hint="Piezas ya enviadas contra la cantidad original.">
                    <div class="mb-2 flex items-baseline justify-between text-sm">
                        ...
                        <span class="font-bold tabular-nums text-slate-900 dark:text-white">{{ $progress }}%</span>
                    </div>
                    <div class="h-3 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700"
                        role="progressbar" aria-valuenow="{{ min($progress, 100) }}" aria-valuemin="0" aria-valuemax="100"
                        aria-label="Avance de envío de la orden">
                ...
                <x-ui.section title="Historial de estados" hint="Los últimos cinco cambios, del más reciente al más antiguo.">
                            ...
                            @foreach ($workOrder->statusLogs->sortByDesc('created_at')->take(5) as $log)
                                <li wire:key="wo-log-{{ $log->id }}" class="relative pb-6">
                                    ...
                                    <div class="relative flex gap-3">
                                        <span class="flex size-8 shrink-0 items-center justify-center rounded-full ring-4 ring-white dark:ring-slate-800"
                                            style="background-color: {{ $log->toStatus->color ?? '#6B7280' }}" aria-hidden="true">
                                            <svg class="size-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                        </span>
                                        <div class="flex min-w-0 flex-1 flex-wrap items-baseline justify-between gap-x-4 gap-y-1 pt-1.5">
                                            <p class="text-xs text-slate-500 dark:text-slate-400">
                                                @if ($log->fromStatus)
                                                    <span class="font-semibold" style="color: {{ $log->fromStatus->color }}">{{ $log->fromStatus->name }}</span> →
                                                @else
                                                    Creado como
                                                @endif
                                                <span class="font-semibold" style="color: {{ $log->toStatus->color }}">{{ $log->toStatus->name }}</span>
                                                @if ($log->user)
                                                    por <span class="font-semibold text-slate-900 dark:text-white">{{ $log->user->name }}</span>
                                                @endif
                                            </p>
                                            <span class="whitespace-nowrap text-xs tabular-nums text-slate-400 dark:text-slate-500">{{ $log->created_at->format('d/m/Y H:i') }}</span>
                                        </div>
                                    </div>
                                </li>
                            @endforeach
        ...
        <x-ui.table id="wo-panel-lots" role="tabpanel" aria-labelledby="wo-tab-lots" tabindex="0"
            ...
            @forelse ($workOrder->lots as $lot)
                @php
                    ...
                    $dotHex = ['gray' => '#d1d5db', 'yellow' => '#fbbf24', 'green' => '#22c55e', 'blue' => '#3b82f6', 'orange' => '#f97316'];
                ...
                <tr wire:key="lot-{{ $lot->id }}" class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                    ...
                    <td class="whitespace-nowrap px-4 py-3 text-right align-top tabular-nums text-slate-900 dark:text-white">
                    ...
                    <td class="px-4 py-3 align-top">
                        @if ($lot->crimpLots->isEmpty())
                            <span class="whitespace-nowrap text-xs text-slate-400 dark:text-slate-500">Sin lotes de CRIMP</span>
                        @else
                            <div class="flex max-w-[210px] flex-wrap gap-1">
                                @foreach ($lot->crimpLots as $cl)
                                    <x-ui.badge wire:key="crimp-{{ $cl->id }}" tone="accent">{{ $cl->crimp_lot_number }}</x-ui.badge>
                                @endforeach
                            </div>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-3 align-top">
                        ...
                        @if ($prodBad > 0)
                            <div class="mt-0.5 text-[10px] font-semibold text-red-600 dark:text-red-400">{{ number_format($prodBad) }} malas</div>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-3 align-top">
                        <div class="mb-1.5 flex items-center gap-2 whitespace-nowrap">
                            ...
                            <span class="text-xs font-medium text-slate-700 dark:text-slate-200">{{ $qText }}</span>
                        </div>
                        <div class="h-1.5 w-24 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                    ...
                    <td class="px-4 py-3 align-top">
                        <div class="mb-1.5 flex items-center gap-2 whitespace-nowrap">
                            <span class="inline-block size-3 shrink-0 rounded-full"
                                ...
                                aria-hidden="true"></span>
                            <span class="text-xs font-medium text-slate-700 dark:text-slate-200">{{ $pText }}</span>
                        </div>
                        <div class="h-1.5 w-24 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                    ...
                    <td class="px-4 py-3 align-top">
                        <div class="mb-1 flex items-center gap-1.5 whitespace-nowrap">
                            @if ($lotProgress['done'])
                                <svg class="size-4 shrink-0 text-green-500" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd"/></svg>
                            @endif
                ...
                <tr>
                    <td colspan="9">
                        <x-ui.empty icon="box" title="Todavía no hay lotes"
                            ...
                            <x-slot:action>
                                <x-ui.btn variant="primary" wire:click="openCreateLotModal">Agregar el primer lote</x-ui.btn>
                            </x-slot:action>
                        </x-ui.empty>
                    </td>
                </tr>
    ...
    @if ($activeTab === 'weighings')
        <x-ui.table id="wo-panel-weighings" role="tabpanel" aria-labelledby="wo-tab-weighings" tabindex="0"
            title="Pesadas de esta orden de trabajo"
            hint="Cada pesada descuenta piezas del pendiente de su lote.">
            <x-slot:aside>
            ...
            @forelse ($weighings as $weighing)
                <tr wire:key="weighing-{{ $weighing->id }}" class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                    <td class="whitespace-nowrap px-4 py-3 text-slate-900 dark:text-white">{{ $weighing->weighed_at?->format('d/m/Y H:i') ?? '—' }}</td>
                    <td class="whitespace-nowrap px-4 py-3 font-semibold text-slate-900 dark:text-white">{{ $weighing->lot?->lot_number ?? '—' }}</td>
                    <td class="whitespace-nowrap px-4 py-3 text-right font-semibold tabular-nums text-green-700 dark:text-green-300">{{ number_format($weighing->good_pieces) }}</td>
                    <td class="whitespace-nowrap px-4 py-3 text-right font-semibold tabular-nums text-red-700 dark:text-red-300">{{ number_format($weighing->bad_pieces) }}</td>
                    <td class="whitespace-nowrap px-4 py-3 text-right font-bold tabular-nums text-slate-900 dark:text-white">{{ number_format($weighing->good_pieces + $weighing->bad_pieces) }}</td>
                    <td class="whitespace-nowrap px-4 py-3 text-slate-600 dark:text-slate-300">{{ $weighing->weighedBy?->name ?? '—' }}</td>
                    <td class="px-4 py-3">
    ...
    @if ($showLotModal)
        <x-ui-modal wire:key="modal-lot-{{ $editingLotId ?? 'new' }}"
            :title="$editingLotId ? 'Editar lote' : 'Agregar lote'"
                ...
                <x-ui.section step="1" title="Identificación y cantidad"
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <x-ui.field label="Número de lote" required :error="$errors->first('lotNumber')">
                            <input type="text" wire:model="lotNumber" class="w-full" placeholder="Ej: 001" required>
                        </x-ui.field>

                        <x-ui.field label="Cantidad" required
                            hint="Piezas que lleva este lote."
                            :error="$errors->first('lotQuantity')">
                ...
                @if ($editingLotId)
                    <x-ui.section step="2" title="Estado y notas" class="mt-5">
                        ...
                        <x-ui.field label="Comentarios" optional class="mt-4"
                            hint="Uso interno; no se imprime." :error="$errors->first('lotComments')">
    ...
    @if ($showDeleteLotConfirm)
        ...
        <x-ui-modal wire:key="modal-delete-lot"
            :title="'Eliminar el lote '.($lotToDelete?->lot_number ?? '')"
            ...
            <x-slot:footer>
                ...
                <x-ui.btn variant="danger" wire:click="deleteLot" wire:loading.attr="disabled" wire:target="deleteLot">Eliminar lote</x-ui.btn>
            </x-slot:footer>
        </x-ui-modal>
    @endif
    ...
    @if ($showWeighingModal)
        ...
        <x-ui-modal wire:key="modal-weighing-{{ $editingWeighingId ?? 'new' }}"
            :title="$editingWeighingId ? 'Editar pesada' : 'Registrar pesada'"
            subtitle="Cada pesada descuenta piezas del pendiente del lote."
            close="closeWeighingModal" maxWidth="3xl">
            ...
            <form wire:submit="saveWeighing">
                <x-ui.section step="1" title="Lote" hint="Elige el viajero al que pertenecen estas piezas.">
                    <x-ui.field label="Lote" required :error="$errors->first('weighingLotId')">
                        <select wire:model.live="weighingLotId" class="w-full" required>
                            ...
                            @foreach ($workOrder->lots as $lot)
                                <option value="{{ $lot->id }}">{{ $lot->lot_number }} — {{ number_format($lot->quantity) }} pz</option>
                            @endforeach
                        </select>
                    </x-ui.field>
                    ...
                    @if ($weighingLotId)
                        <x-ui.stats cols="3" class="mt-4">
                            ...
                            <x-ui.stat label="Pendiente" :value="number_format($remainingPieces)"
                                :tone="$remainingPieces > 0 ? 'warn' : 'good'" />
                        </x-ui.stats>
                    @endif
                </x-ui.section>
                ...
                <x-ui.section step="3" title="Cuándo y notas" class="mt-5">
                    ...
                    <x-ui.field label="Comentarios" optional class="mt-4"
                        hint="Contexto de la pesada: turno, incidencia, etc."
                        :error="$errors->first('weighingComments')">
            ...
            <x-slot:footer>
                ...
                <x-ui.btn variant="primary" wire:click="saveWeighing" wire:loading.attr="disabled" wire:target="saveWeighing">
    ...
    @if ($showDeleteWeighingConfirm)
        ...
        <x-ui-modal wire:key="modal-delete-weighing" title="Eliminar pesada"
            subtitle="Esta acción no se puede deshacer."
            close="$set('showDeleteWeighingConfirm', false)" maxWidth="lg">
            ...
            <x-slot:footer>
                <x-ui.btn variant="secondary" wire:click="$set('showDeleteWeighingConfirm', false)">Cancelar</x-ui.btn>
                <x-ui.btn variant="danger" wire:click="deleteWeighing" wire:loading.attr="disabled" wire:target="deleteWeighing">Eliminar pesada</x-ui.btn>
            </x-slot:footer>
        </x-ui-modal>
    @endif

    {{-- El modal de firma está deshabilitado. No se deja el tag comentado
         porque Livewire parsea <livewire:...> antes de descartar el comentario
         Blade y el nombre con puntos rompe el validador de tags. --}}
    ...
    <div class="mt-6">
        @livewire(\App\Livewire\Admin\History\HistoryExplorer::class, [
            'entityType' => \App\Models\WorkOrder::class,
