...
@if (count($rows) > 0)
    <div class="border-2 border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <table class="w-full text-xs">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @foreach ($rows as $row)
                    <tr wire:key="pkg-w-{{ $type }}-{{ $row['id'] }}">
                        ...
                        <td class="px-3 py-2 text-right font-medium {{ $accentText }}">
                            {{ number_format($row['quantity']) }}</td>
                        <td class="px-3 py-2 text-right text-gray-600 dark:text-gray-400">
                            {{ $row['weight'] !== null ? number_format($row['weight'], 3) : '—' }}</td>
                        <td class="px-3 py-2 text-gray-600 dark:text-gray-400">{{ $row['weighed_by'] }}</td>
                        <td class="px-3 py-2 text-gray-500 dark:text-gray-400 max-w-xs truncate">
                            {{ $row['comments'] ?? '-' }}</td>
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                @foreach ($rows as $row)
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                        <td class="whitespace-nowrap px-3 py-2 text-slate-700 dark:text-slate-300">
                            {{ $row['weighed_at'] ?? '—' }}</td>
                        <td class="px-3 py-2 text-slate-700 dark:text-slate-300">{{ $row['crimp_lot'] ?? '—' }}</td>
                        <td class="px-3 py-2 text-right font-semibold tabular-nums {{ $accentText }}">
                            {{ number_format($row['quantity']) }}</td>
                        <td class="px-3 py-2 text-right tabular-nums text-slate-600 dark:text-slate-400">
                            {{ $row['weight'] !== null ? number_format($row['weight'], 3) : '—' }}</td>
                        <td class="px-3 py-2 text-slate-600 dark:text-slate-400">{{ $row['weighed_by'] }}</td>
                        <td class="max-w-xs truncate px-3 py-2 text-slate-500 dark:text-slate-400">
                            {{ $row['comments'] ?? '—' }}</td>
                        <td class="px-3 py-2">
            ...
            <tfoot
                class="border-t border-slate-200 bg-slate-50 font-semibold dark:border-slate-700 dark:bg-slate-900/50">
                <tr>
