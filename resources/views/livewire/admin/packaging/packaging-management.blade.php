<x-ui.page eyebrow="Empaque" title="Gestión de empaque"
    ...
    <x-ui.stats cols="5">
        <x-ui.stat label="Listos sin empacar" :value="number_format($stats['listos_sin_empacar'])" :tone="$stats['listos_sin_empacar'] > 0 ? 'warn' : 'good'"
        ...
        <x-ui.stat label="Sobrante declarado" :value="number_format($stats['sobrante'])" unit="pz" :tone="$stats['sobrante'] > 0 ? 'warn' : 'neutral'"
                    ...
                    <div wire:key="pend-{{ $lot->id }}"
                        ...
                        <div class="min-w-0">
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="text-sm font-bold text-slate-900 dark:text-white">Viajero
                                    {{ $lot->lot_number }}</span>
    ...
    {{-- Filtros --}}
    <x-ui.section title="Buscar registros"
        hint="Filtra por viajero, orden, parte, quien empacó o el comentario del registro.">
        ...
        @forelse ($records as $record)
            ...
            <tr wire:key="rec-{{ $record->id }}" class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                ...
                <td class="px-4 py-3 text-right">
                    <span
                        class="block font-semibold tabular-nums {{ $sobrante > 0 ? 'text-orange-700 dark:text-orange-300' : 'text-slate-500 dark:text-slate-400' }}">
                ...
                <td class="px-4 py-3">
                    <span
                        class="block text-sm text-slate-700 dark:text-slate-200">{{ $record->packedBy?->name ?? '—' }}</span>
                    <span class="block text-xs text-slate-500 dark:text-slate-400">
                ...
                <td class="px-4 py-3">
                    <x-ui.row-actions label="el registro del viajero {{ $lot?->lot_number ?? '' }}">
                        <x-ui.icon-btn tone="primary"
                            label="Editar el registro del viajero {{ $lot?->lot_number ?? '' }}"
                        ...
                        @if ($bloqueo)
                            <span
                                class="ui-icon-btn cursor-not-allowed bg-slate-100 text-slate-300 dark:bg-slate-700 dark:text-slate-500"
                        ...
                        @else
                            <x-ui.icon-btn tone="danger"
                                label="Eliminar el registro del viajero {{ $lot?->lot_number ?? '' }}"
    ...
    @if ($showModal)
        ...
        <x-ui-modal wire:key="modal-packaging-{{ $editingId ?? 'new' }}" :title="$editingId ? 'Editar registro de empaque' : 'Nuevo registro de empaque'"
            ...
            <x-ui.section step="1" title="¿De qué viajero es este empaque?"
                ...
                @if ($lotSel)
                    <div
                        class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-800">
                        <div class="min-w-0">
                            <span class="block text-sm font-bold text-slate-900 dark:text-white">Viajero
                                {{ $lotSel->lot_number }}</span>
                            <span class="block text-xs text-slate-500 dark:text-slate-400">
                                WO {{ $poSel?->wo ?? '—' }} · Parte {{ $poSel?->part?->number ?? '—' }}
                            </span>
                        </div>
                        @unless ($editingId)
                            <x-ui.btn variant="ghost" size="sm" wire:click="clearLotSelection">Cambiar
                                viajero</x-ui.btn>
                        @endunless
                    </div>
                    ...
                    @if ($poSel?->part?->is_crimp)
                        <x-ui.note tone="warn" class="mt-3">
                            Este viajero es <strong>CRIMP</strong>: su empaque normalmente se registra con doble
                            pesada en «Pesadas de Empaque». Usa esta pantalla sólo si sabes que le corresponde un
                            registro simple.
                        </x-ui.note>
                    @endif
                    ...
                    @if ($cerrado)
                        <x-ui.note tone="warn" class="mt-3">
                            El viajero ya fue cerrado o recibido por Materiales. Puedes corregir las cantidades,
                            pero la decisión que ya se tomó <strong>no se recalcula sola</strong>: avisa a
                            Materiales si cambian.
                        </x-ui.note>
                    @endif
                @else
                    ...
                    @if ($lotsForCreate->isEmpty())
                        <x-ui.note tone="info" class="mt-3">
                            No hay viajeros listos para empacar. Calidad tiene que aprobar piezas antes de que
                            Empaque pueda registrar algo.
                        </x-ui.note>
                    @endif
                @endif
            ...
            @if ($lotSel)
                <x-ui.section step="2" title="¿Cuántas piezas empacaste?" tone="accent"
                    ...
                    <x-ui.stats cols="3" class="mb-4">
                        ...
                        <x-ui.stat label="Queda por registrar" :value="number_format($restante)" unit="pz" :tone="$restante > 0 ? 'warn' : 'good'"
                            help="Aprobadas menos lo que ya está empacado o declarado como sobrante en otros registros." />
                        <x-ui.stat label="Quedaría sin registrar" :value="number_format($this->modalLeftover)" unit="pz" :tone="$this->modalLeftover > 0 ? 'warn' : 'good'"
                            help="Lo que faltaría por capturar si guardas con estos números." />
                    </x-ui.stats>

                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <x-ui.field label="Piezas empacadas" required :hint="'Máximo disponible: ' . number_format($restante) . ' pz.'" :error="$errors->first('formPackedPieces')">
                            <input type="number" min="1" step="1"
                                wire:model.live.debounce.400ms="formPackedPieces"
                        ...
                        <x-ui.field label="Piezas sobrantes" optional
                            ...
                            <input type="number" min="0" step="1"
                                wire:model.live.debounce.400ms="formSurplusPieces"
                                class="w-full text-right tabular-nums">
                        </x-ui.field>
                    </div>

                    <x-ui.field label="Fecha y hora del empaque" required class="mt-4"
                        hint="No puede estar en el futuro." :error="$errors->first('formPackedAt')">
                ...
                <x-ui.section step="3" title="Corrección del sobrante y notas"
                    ...
                    <x-ui.field label="Sobrante corregido" optional
                        ...
                        <input type="number" min="0" step="1"
                            wire:model.live.debounce.400ms="formAdjustedSurplus"
                    ...
                    <x-ui.field label="Comentarios" optional class="mt-4" :error="$errors->first('formComments')">
                        <textarea wire:model="formComments" rows="2" class="w-full" placeholder="Observaciones del empaque..."></textarea>
                    </x-ui.field>
                </x-ui.section>
            @endif
    ...
    @if ($showDeleteModal && $this->deletingRecord)
        ...
        <x-ui-modal wire:key="modal-delete-packaging" title="Eliminar registro de empaque" :subtitle="'Viajero ' . ($recDel->lot?->lot_number ?? '—')"
