            ...
            <x-ui.field label="Texto a buscar">
                <div class="relative">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search"
                        placeholder="WO, viajero o parte..." class="w-full pl-10">
                </div>
            </x-ui.field>
    ...
    <x-ui.table>
        <x-slot:head>
            <tr>
                ...
                <x-ui.th align="right">Obj. CRIMP</x-ui.th>
                <x-ui.th align="right">Manguitas</x-ui.th>
                <x-ui.th align="right">CRIMP</x-ui.th>
                <x-ui.th align="right"># Pesadas</x-ui.th>
        ...
        @forelse ($lots as $lot)
            ...
            @endphp
            <tr wire:key="wlot-{{ $lot->id }}" class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                <td class="whitespace-nowrap px-4 py-3 font-semibold text-sky-700 dark:text-sky-300">{{ $po->wo ?? '—' }}</td>
                <td class="whitespace-nowrap px-4 py-3 font-semibold text-slate-900 dark:text-white">{{ $lot->lot_number }}</td>
                <td class="whitespace-nowrap px-4 py-3 text-slate-600 dark:text-slate-300">{{ $part->number ?? '—' }}</td>
                <td class="px-4 py-3 text-right tabular-nums text-slate-600 dark:text-slate-300">{{ number_format($target) }}</td>
                <td class="px-4 py-3 text-right font-semibold tabular-nums text-sky-700 dark:text-sky-400">{{ number_format($pieces) }}</td>
                <td class="px-4 py-3 text-right font-semibold tabular-nums text-amber-700 dark:text-amber-400">{{ number_format($crimp) }}</td>
                <td class="px-4 py-3 text-right tabular-nums text-slate-600 dark:text-slate-300">{{ number_format($count) }}</td>
                <td class="px-4 py-3">
                    <div class="flex justify-end">
                        <x-ui.btn variant="secondary" size="sm" wire:click="openDetailModal({{ $lot->id }})">
        ...
        @empty
            <tr>
                <td colspan="8">
                    <x-ui.empty icon="box" title="No hay pesadas de empaque registradas"
                        hint="Las pesadas aparecerán aquí cuando Empaque confirme manguitas o CRIMP." />
                </td>
            </tr>
        @endforelse
=======
...
>>>>>>> dba4729e63772ad9744b5ced48b25a0fdb214628
    ...
    @if ($showDetailModal && $selectedLot)
        <x-ui-modal wire:key="modal-packaging-detail" title="Detalle de pesadas — viajero"
            subtitle="Manguitas y CRIMP registrados para este viajero."
            close="closeDetailModal" maxWidth="4xl">
            ...
            <x-ui.section title="Pesadas de manguitas" hint="{{ count($pieceWeighings) }} {{ Str::plural('pesada', count($pieceWeighings)) }} registrada{{ count($pieceWeighings) === 1 ? '' : 's' }}.">
                @include('livewire.admin.packaging.partials.weighing-table', ['rows' => $pieceWeighings, 'type' => 'piece', 'accentText' => 'text-sky-700 dark:text-sky-400', 'total' => $pieceTotal])
            </x-ui.section>

            <x-ui.section title="Pesadas de CRIMP" hint="{{ count($crimpWeighings) }} {{ Str::plural('pesada', count($crimpWeighings)) }} registrada{{ count($crimpWeighings) === 1 ? '' : 's' }}.">
                @include('livewire.admin.packaging.partials.weighing-table', ['rows' => $crimpWeighings, 'type' => 'crimp', 'accentText' => 'text-amber-700 dark:text-amber-400', 'total' => $crimpTotal])
            </x-ui.section>
    ...
    @if ($showEditModal)
        <x-ui-modal wire:key="modal-packaging-edit"
            :badge="$editType === 'crimp' ? 'CRIMP' : 'Manguitas'"
            :title="'Editar pesada de '.($editType === 'crimp' ? 'CRIMP' : 'manguitas')"
            close="closeEditModal" maxWidth="lg">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <x-ui.field label="Cantidad" required :error="$errors->first('editQuantity')">
                            <input wire:model="editQuantity" type="number" min="1" class="w-full" placeholder="0">
                        </x-ui.field>
                        <x-ui.field label="Peso (kg)" optional :error="$errors->first('editWeight')">
                            <input wire:model="editWeight" type="number" step="0.001" min="0" class="w-full" placeholder="opcional">
                        </x-ui.field>
                    </div>
