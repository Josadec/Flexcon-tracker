<div class="space-y-6">
    ...
    @if (session()->has('message'))
        <div class="fixed top-4 right-4 z-50 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white px-4 py-3 rounded-lg shadow-lg">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Verificación de pesadas de producción</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-lg p-4">
            <div class="text-xs text-gray-500 dark:text-gray-400 mb-1">Total con Pesadas</div>
        ...
        <div class="bg-white dark:bg-gray-800 border-2 border-yellow-200 dark:border-yellow-700 rounded-lg p-4">
            ...
            <div class="text-2xl font-semibold text-yellow-700 dark:text-yellow-300">{{ $stats['pending'] }}</div>
        </div>
        <div class="bg-white dark:bg-gray-800 border-2 border-green-200 dark:border-green-700 rounded-lg p-4">
            ...
            <div class="text-2xl font-semibold text-green-700 dark:text-green-300">{{ $stats['completed'] }}</div>
        </div>
        <div class="bg-white dark:bg-gray-800 border-2 border-red-200 dark:border-red-700 rounded-lg p-4">
            <div class="text-xs text-red-600 dark:text-red-400 mb-1">Con Rechazos</div>
            <div class="text-2xl font-semibold text-red-700 dark:text-red-300">{{ $stats['rejected'] }}</div>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Cant. Lote</th>
                        <th class="px-6 py-3 text-right text-xs font-semibold text-green-600 dark:text-green-400 uppercase">Prod. Buenas</th>
                        <th class="px-6 py-3 text-right text-xs font-semibold text-teal-600 dark:text-teal-400 uppercase">Cal. Aprobadas</th>
                        <th class="px-6 py-3 text-right text-xs font-semibold text-red-600 dark:text-red-400 uppercase">Cal. Rechazadas</th>
                        <th class="px-6 py-3 text-right text-xs font-semibold text-yellow-600 dark:text-yellow-400 uppercase">Pendientes</th>
                        <th class="px-6 py-3 text-center text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Estado</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($lots as $lot)
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                            <td class="px-6 py-4 text-blue-600 dark:text-blue-400 font-medium">{{ $po->wo }}</td>
                            <td class="px-6 py-4 text-gray-900 dark:text-white font-medium">{{ $lot->lot_number }}</td>
                            <td class="px-6 py-4 text-gray-700 dark:text-gray-300">{{ $part->number }}</td>
                            <td class="px-6 py-4 text-right text-gray-700 dark:text-gray-300">{{ number_format($lot->quantity) }}</td>
                            <td class="px-6 py-4 text-right font-medium text-green-600 dark:text-green-400">{{ number_format($prodGood) }}</td>
                            <td class="px-6 py-4 text-right font-medium text-teal-600 dark:text-teal-400">{{ number_format($qualGood) }}</td>
                            <td class="px-6 py-4 text-right font-medium text-red-600 dark:text-red-400">{{ number_format($qualBad) }}</td>
                            <td class="px-6 py-4 text-right font-bold {{ $qualPendingPcs > 0 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400' }}">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    @endphp
                                    <span class="inline-block w-4 h-4 rounded border-2 border-gray-200 dark:border-gray-600 {{ $semColor }}" title="Calidad: {{ ucfirst($semaphore) }}"></span>
                                    @if ($hasRejected)
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-700">
                                            Descarte
                                        </span>
                                    @endif
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-4 text-base font-medium text-gray-900 dark:text-white">No hay lotes con pesadas de producción</p>
                                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Los lotes aparecerán aquí cuando Producción registre pesadas.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <div class="relative z-10 inline-block align-bottom bg-white dark:bg-gray-800 text-left overflow-hidden transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border-2 border-gray-200 dark:border-gray-700 rounded-lg">
                    ...
                    <div class="px-6 py-4 space-y-6 max-h-[70vh] overflow-y-auto">
                        ...
                        <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
                            ...
                            <div class="bg-green-50 dark:bg-green-900/20 border-2 border-green-200 dark:border-green-700 p-3 rounded-lg text-center">
                            ...
                            <div class="bg-teal-50 dark:bg-teal-900/20 border-2 border-teal-200 dark:border-teal-700 p-3 rounded-lg text-center">
                            ...
                            <div class="bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-700 p-3 rounded-lg text-center">
                            ...
                            <div class="{{ $qualPending > 0 ? 'bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-200 dark:border-yellow-700' : 'bg-green-50 dark:bg-green-900/20 border-2 border-green-200 dark:border-green-700' }} p-3 rounded-lg text-center">
                        ...
                        <div>
                            ...
                            @else
                                <p class="text-sm text-gray-500 dark:text-gray-400">Sin pesadas de producción.</p>
                            @endif
                        </div>
                    ...
                    <div class="px-6 py-4 border-t-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex flex-col sm:flex-row gap-3 sm:justify-end">
                        ...
                        @if ($qualPending > 0)
                            <button wire:click="openWeighingModal"
                                class="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white font-medium rounded-md transition-colors flex items-center gap-2">
            ...
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="relative z-10 inline-block align-bottom bg-white dark:bg-gray-800 text-left overflow-hidden transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border-2 border-gray-200 dark:border-gray-700 rounded-lg">
                    ...
                    <div class="px-6 py-4 space-y-4">
                        {{-- Pendiente --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Pendiente de Verificar</label>
                            <div class="w-full px-3 py-2 border-2 border-teal-300 dark:border-teal-600 rounded-lg font-bold text-lg text-center bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Piezas Aprobadas *</label>
                                <input wire:model="qualGoodPieces" type="number" min="0"
                                    class="w-full px-4 py-2 border-2 border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm rounded-md focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500"
                                    placeholder="0">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Piezas Rechazadas *</label>
                                <input wire:model="qualBadPieces" type="number" min="0"
                                    class="w-full px-4 py-2 border-2 border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm rounded-md focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500"
                                    placeholder="0">
                                @error('qualBadPieces')
                                    <span class="text-xs text-red-600 dark:text-red-400 mt-1 block">{{ $message }}</span>
                                @enderror
                        ...
                        {{-- Discard warning --}}
                        @if ($qualBadPieces > 0)
                            <div class="bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 p-3 rounded-lg">
                                <div class="flex items-center text-sm text-red-700 dark:text-red-300">
                                    ...
                                    {{ number_format($qualBadPieces) }} piezas rechazadas serán descartadas.
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="px-6 py-4 border-t-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex gap-3 justify-end">
                        ...
                        <button wire:click="saveQualityWeighing"
                            class="px-4 py-2 {{ $editingQualityWeighingId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-teal-600 hover:bg-teal-700' }} text-white font-medium rounded-md transition-colors">
