<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <h1 class="mt-3 text-2xl font-semibold text-gray-900 dark:text-white">Editar estado de produccion</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Ajusta la configuracion del estado sin perder su historial de uso.
            </p>
        </div>

        <a
            href="{{ route('admin.production-statuses.show', $productionStatus) }}"
            wire:navigate
            class="inline-flex items-center gap-2 self-start rounded-md border-2 border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:border-gray-300 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:border-gray-600 dark:hover:bg-gray-700/60"
        >
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <form wire:submit="update" class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div class="rounded-lg border-2 border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
            <div class="border-b border-gray-200 px-4 py-4 dark:border-gray-700">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Mantiene la misma estructura visual que el resto del admin.
                </p>
            </div>
        ...
        <div class="space-y-6">
            <div class="rounded-lg border-2 border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                ...
                <div class="mt-4 flex items-center gap-3">
                    ...
                    <div>
                        <div class="text-sm font-semibold text-gray-900 dark:text-white">{{ $name ?: 'Estado' }}</div>
            ...
            <div class="rounded-lg border-2 border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                <h2 class="text-base font-semibold text-gray-900 dark:text-white">Contexto</h2>
                <div class="mt-3 space-y-3 text-sm text-gray-600 dark:text-gray-300">
                    <p>Creado: {{ $productionStatus->created_at->format('d/m/Y H:i') }}</p>
                    <p>Actualizado: {{ $productionStatus->updated_at->format('d/m/Y H:i') }}</p>
                    <p>Si el estado ya esta en uso, editarlo evita duplicar catalogos innecesariamente.</p>
                </div>
            </div>
        </div>
    </form>
