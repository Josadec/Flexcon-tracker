    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 pb-20">
        ...
        @if ($workOrdersGrouped->isEmpty())
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-12 text-center">
                ...
                {{-- Vacío porque no hay nada, o vacío porque todo está terminado:
                     no es lo mismo y el tablero tiene que decir cuál de los dos. --}}
                @if (! $showingFinished && $totalFinishedHidden > 0)
                    <h3 class="mt-4 text-base font-medium text-gray-900 dark:text-white">
                        Todo el trabajo visible está terminado
                    </h3>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        Hay <strong class="text-gray-700 dark:text-gray-200">{{ $totalFinishedHidden }}</strong>
                        {{ Str::plural('viajero', $totalFinishedHidden) }}
                        {{ Str::plural('terminado', $totalFinishedHidden) }} fuera de la vista.
                    </p>
                    <button type="button" wire:click="toggleFinished"
                        class="mt-4 inline-flex items-center gap-2 rounded-lg bg-slate-800 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-700 dark:bg-slate-200 dark:text-slate-900 dark:hover:bg-white">
                        Ver terminados
                    </button>
                @else
