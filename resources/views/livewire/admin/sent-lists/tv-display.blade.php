<div
    ...
    class="h-screen bg-white text-slate-900 p-4 flex flex-col overflow-hidden"
...
>
    ...
    <div class="flex items-center justify-between mb-3 flex-shrink-0">
        <div class="flex items-center gap-3">
            <h1 class="text-2xl font-bold text-slate-800 tracking-wide">FLEXCON — Monitor de Producción</h1>
            <span class="text-sm text-slate-600 bg-slate-100 px-2.5 py-1 rounded font-semibold">{{ count($woCards) }} WOs</span>
        </div>
    ...
    <div class="flex items-stretch gap-3 mb-3 flex-shrink-0">
        ...
        @php
            ...
            $pctBg = $pct >= 90 ? 'bg-green-50 border-green-300' : ($pct >= 60 ? 'bg-amber-50 border-amber-300' : 'bg-red-50 border-red-300');
            $pctText = $pct >= 90 ? 'text-green-700' : ($pct >= 60 ? 'text-amber-700' : 'text-red-700');
            $pctBar = $pct >= 90 ? 'bg-green-500' : ($pct >= 60 ? 'bg-amber-400' : 'bg-red-500');
        @endphp
        <div class="flex-1 border-2 {{ $pctBg }} rounded-lg px-4 py-2 flex flex-col justify-center">
            <div class="flex items-center justify-between mb-1">
                <span class="text-sm font-bold uppercase tracking-wider text-slate-600">% Cumplimiento global</span>
        ...
        <div class="flex-1 border-2 border-indigo-300 bg-indigo-50 rounded-lg px-4 py-2 flex flex-col justify-center">
            <div class="flex items-center justify-between mb-1">
                <span class="text-sm font-bold uppercase tracking-wider text-slate-600">Empacadas Hoy</span>
                <span class="text-3xl font-extrabold text-cyan-700">{{ number_format($dayKpis['packed_today_total']) }}</span>
            </div>
            <div class="flex items-center gap-3 text-xs font-semibold text-slate-700 mt-1">
                @foreach (['Mesa' => 'bg-sky-500', 'Máquina' => 'bg-green-500', 'Semi-Automática' => 'bg-cyan-500'] as $st => $colorClass)
    ...
    <div class="flex-1 flex flex-col min-h-0">
            ...
            @foreach ($slides as $slideIndex => $slideCards)
                ...
                >
                    @foreach ($slideCards as $card)
                        <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-0">
                            {{-- Card Header --}}
                            <div class="px-3 py-2 bg-slate-50 border-b border-slate-200 flex-shrink-0">
                                <div class="flex items-center justify-between gap-3">
                                    <div class="flex items-center gap-3 min-w-0">
                                        <span class="text-xl font-bold text-indigo-600">{{ $card['wo'] }}</span>
                                        <span class="text-sm text-slate-500">{{ $card['item'] }}</span>
                                        <span class="text-sm font-bold text-slate-800">#{{ $card['part_number'] }}</span>
                                        @if ($card['is_crimp'])
                                            <span class="px-1.5 py-0.5 text-xs font-bold bg-cyan-100 text-cyan-700 rounded">CRIMP</span>
                                        @endif
                                        <span class="text-sm text-slate-500 truncate max-w-[280px]">{{ $card['description'] }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 flex-shrink-0">
                                        ...
                                        <span class="text-sm font-semibold text-slate-600 bg-white px-2 py-1 rounded border border-slate-200">{{ $card['lot_count'] }} lotes</span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <table class="w-full text-sm">
                                    ...
                                    <tbody class="divide-y divide-gray-100">
                                        @foreach ($card['lots'] as $li => $lot)
                                            <tr class="{{ $li % 2 === 0 ? 'bg-white' : 'bg-slate-50/50' }}">
                                                {{-- Lot number --}}
                                                <td class="px-3 py-2 font-bold text-base text-slate-800 whitespace-nowrap">
                                                    {{ $lot['lot_number'] }}
                                                ...
                                                {{-- Pieces target --}}
                                                <td class="px-3 py-2 text-right text-slate-700 font-mono font-semibold text-base whitespace-nowrap">{{ number_format($lot['quantity']) }}</td>
                                                        ...
                                                        @if (!empty($lot['crimp_lots']))
                                                            @foreach ($lot['crimp_lots'] as $cl)
                                                                ...
                                                                <span class="inline-flex items-center gap-1 mr-2">
                                                                    ...
                                                                    <span class="text-slate-700 font-semibold">{{ $cl['crimp_lot_number'] }}</span>
                                                                </span>
                                                            @endforeach
                                                        @else
                                                ...
                                                <td class="px-3 py-2">
                                                    <div class="flex items-center gap-2">
                                                        <div class="flex-1 bg-slate-200 rounded-full h-2.5 overflow-hidden">
                                                            <div class="h-full rounded-full {{ $lot['prod_pct'] >= 100 ? 'bg-green-500' : ($lot['prod_pct'] > 0 ? 'bg-amber-400' : 'bg-slate-300') }}"
                                                                style="width:{{ max($lot['prod_pct'], 2) }}%"></div>
                                                        </div>
                                                        <span class="font-mono text-sm font-semibold {{ $lot['prod_pct'] >= 100 ? 'text-green-600' : 'text-slate-700' }} whitespace-nowrap w-24 text-right">{{ number_format($lot['prod_weighed']) }}/{{ number_format($lot['quantity']) }}</span>
                                                    </div>
                                                </td>
                                                {{-- Quality mini bar --}}
                                                <td class="px-3 py-2">
                                                    <div class="flex items-center gap-2">
                                                        <div class="flex-1 bg-slate-200 rounded-full h-2.5 overflow-hidden">
                                                            <div class="h-full rounded-full {{ $lot['qual_pct'] >= 100 ? 'bg-green-500' : ($lot['qual_pct'] > 0 ? 'bg-sky-400' : 'bg-slate-300') }}"
                                                                style="width:{{ max($lot['qual_pct'], 2) }}%"></div>
                                                        </div>
                                                        <span class="font-mono text-sm font-semibold {{ $lot['qual_pct'] >= 100 ? 'text-green-600' : 'text-slate-700' }} whitespace-nowrap w-24 text-right">{{ number_format($lot['qual_good']) }}/{{ number_format($lot['qual_target']) }}</span>
                                                    </div>
                                                </td>
                                                {{-- Packaging mini bar --}}
                                                <td class="px-3 py-2">
                                                    <div class="flex items-center gap-2">
                                                        <div class="flex-1 bg-slate-200 rounded-full h-2.5 overflow-hidden">
                                                            <div class="h-full rounded-full {{ $lot['pkg_pct'] >= 100 ? 'bg-green-500' : ($lot['pkg_pct'] > 0 ? 'bg-orange-400' : 'bg-slate-300') }}"
                                                                style="width:{{ max($lot['pkg_pct'], 2) }}%"></div>
                                                        </div>
                                                        <span class="font-mono text-sm font-semibold {{ $lot['pkg_pct'] >= 100 ? 'text-green-600' : 'text-slate-700' }} whitespace-nowrap w-24 text-right">{{ number_format($lot['packed']) }}/{{ number_format($lot['pkg_target']) }}</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
        ...
        @else
            <div class="flex-1 flex items-center justify-center">
                <p class="text-slate-400 text-2xl font-semibold">No hay Work Orders activas</p>
            </div>
        @endif
            ...
            <div class="flex items-center justify-center gap-2 mt-2 flex-shrink-0">
                @foreach ($slides as $si => $s)
                    <button @click="goTo({{ $si }})"
                        :class="currentSlide === {{ $si }} ? 'bg-indigo-500 w-6' : 'bg-slate-300 w-2 hover:bg-slate-400'"
